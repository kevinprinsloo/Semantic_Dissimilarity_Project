...
for subject_idx = subject_idx_cluster
    ...
    for condition_idx = 1:length(conditions)
        ...
        listing = dir(fullfile([pc_path,'/',study_name,'/','aRecordings_8Hz','/',subjects{subject_idx},'/'],'*.mat'));
...
%             modulating_signal_holder_converted = modulating_signal_SPL_from_voltage/max(modulating_signal_SPL_from_voltage);
            ...
            %envelope = envelope';
            %modulating_signal_holder_converted = normaliseVector(envelope);
        ...
        if exist([pc_path,'/',study_name,'/','Results_G_SPL_30Hz','/',condition_name,'/',subjects{subject_idx},'/'],'dir') == 0
            mkdir([pc_path,'/',study_name,'/','Results_G_SPL_30Hz','/',condition_name,'/',subjects{subject_idx},'/']);
        end
        % Save Figures and Data
        filename = [pc_path,'/',study_name,'/','Results_G_SPL_30Hz','/',condition_name,'/',subjects{subject_idx},'/',...
            'mTRF_output']; filetype = '.mat';
        save([filename,filetype],'rho','stim','best_labda_selected','best_lambda_hd','recon_stim','recon_stim_best_lambda','stim_model_reshape','time_lags_fw','model_transfored','-v7.3'); clear filename filetype
