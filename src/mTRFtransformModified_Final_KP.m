function [model_t,t,c_t] = mTRFtransformModified_Final_KP(stim,resp,model,fs,map,tmin,tmax,c)
...
model_hd = [c;reshape(model,size(model,1)*size(model,2),size(model,3))];
model_t = (X'*X)*model_hd*inv(y'*y);
...
if map == -1
    model_t = fliplr(model_t);
    %model_t = model_t';
    model_t = permute(model_t, [2 1 3]);
    t = fliplr(-t);
    t = t';
    c_t = fliplr(c_t);
    c_t = c_t';
