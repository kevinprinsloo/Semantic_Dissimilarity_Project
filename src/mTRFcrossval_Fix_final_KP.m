function [r,p,mse,pred,model] = mTRFcrossval_Fix_final_KP(stim,resp,fs,map,tmin,tmax,lambda)
...
elseif map == -1
    x = resp;
    y = stim;
    [tmin,tmax] = deal(tmax,tmin); % tmax = 600 | tmin = 0
else
...
%dim1 = size(x{1},2)*length(tmin:tmax)+size(x{1},2); % ORIG
dim1 = size(x{1},2)*length(tmin:tmax)+1; % Nate added for ABR
dim2 = size(y{1},2);
...
% % % %     M = d-u-l;
% % % %     %% KP add detailed notes here
% % % %     M(1,2) = 0; % Nate added for ABR
% % % %     M(1,1) = 0; % Nate added for ABR
% % % %     M(2,1) = 0; % Nate added for ABR
% % % % else
    M = eye(dim1,dim1);
    M(1,:) = 0; % Nate added for ABR
% % % % end
